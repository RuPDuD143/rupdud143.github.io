...
app.post('/game/click', async (req, res) => {
  ...
  try {
    ...
    revealed.add(index);
    let multiplier = Number(game.multiplier);
    let safeClicks = game.safe_clicks;
    let status = 'active';
    let result = 'safe';

    if (game.mine_positions.includes(index)) {
      status = 'lost';
      result = 'mine';
    } else {
      safeClicks++;
      multiplier = 1 + safeClicks * (game.mine_positions.length / 25);
    ...
    res.json({ result, multiplier, gameOver: status !== 'active' });
  } catch (err) {
